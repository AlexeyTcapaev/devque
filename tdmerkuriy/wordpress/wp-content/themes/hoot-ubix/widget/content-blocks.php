...
$content_blocks_query = new WP_Query( array( 'post_type' => 'page', 'post__in' => $page_ids, 'posts_per_page' => -1, 'orderby' => 'post__in' ) ); // If $page_ids is empty, query returns all posts

// Temporarily remove read more links from excerpts
hootubix_remove_readmore_link();

// Template modification Hook
do_action( 'hootubix_content_blocks_wrap' );
?>
...
<div class="content-blocks-widget-wrap <?php echo sanitize_html_class( $top_class ); ?>">
	<div class="content-blocks-widget-box <?php echo sanitize_html_class( $bottom_class ); ?>">
		<div class="content-blocks-widget">
			...
			/* Display Title */
			if ( $title )
				echo wp_kses_post( apply_filters( 'hootubix_content_blocks_title', $before_title . $title . $after_title, $title, $before_title, $after_title ) );
			?>

			<div class="flush-columns">
				<?php
				global $post;
				if( !empty( $page_ids ) ): foreach( $content_blocks_query->posts as $post ) :
					...
					if ( !empty( $box['excerpt'] ) && empty( $box['url'] ) ) {
						$linktag = '<a href="' . esc_url( get_permalink() ) . '" ' . hybridextend_get_attr( 'content-block-link', 'permalink' ) . '>';
						$linktagend = '</a>';
					...
					?>
					<div class="content-block-column <?php echo 'column-1-' . $columns; ?> <?php echo sanitize_html_class( 'content-block-' . $style ) . ' ' . $column_class; ?>">
						<div class="content-block <?php echo $block_class; ?>">
							...
							<div class="content-block-content<?php
								...
								<h4 class="content-block-title"><?php echo $linktag;
									the_title();
									echo $linktagend; ?></h4>
								<?php if ( !empty( $box['subtitle'] ) ) echo '<h6 class="content-block-subtitle small">' .  wp_kses_post( $box['subtitle'] ) . '</h6>'; ?> 
			...
			<?php
			// Template modification Hook
			do_action( 'hootubix_content_blocks_end' );
			?>
