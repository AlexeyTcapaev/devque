...
function hootubix_theme_customizer_options() {
	...
	$settings['header_layout_help'] = array(
		'section'     => $section,
		'type'        => 'content',
		'content'     => '<em>' . __( 'Select options below to create a variety of header layouts. Example:', 'hoot-ubix' ) . '</em><br><img src="' . $imagepath . 'header-layout.png">',
	);

	$settings['primary_menuarea'] = array(
		'label'       => __( 'Primary Area (right of logo)', 'hoot-ubix' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
			'menu'        => __('Display Primary Menu', 'hoot-ubix'),
	...
	//     add_filter( 'primary_menuarea_custom_allowscript', 'hootubix_child_textarea_allowscript' );
	...
	$settings['secondary_menu_location'] = array(
		'label'       => __( 'Secondary Menu Location', 'hoot-ubix' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
			'top'        => __('Top (above logo)', 'hoot-ubix'),
			'bottom'     => __('Bottom (below logo)', 'hoot-ubix'),
			'none'       => __('Do not display secondary menu', 'hoot-ubix'),
		),
		'default'     => 'bottom',
	);

	$settings['secondary_menu_align'] = array(
		'label'       => __( 'Secondary Menu Alignment', 'hoot-ubix' ),
		'section'     => $section,
		'type'        => 'radio',
		'choices'     => array(
			...
			'options'     => array(
				...
				'type' => array(
					...
					'choices' => array(
						...
						'image'       => __('Background Image', 'hoot-ubix'),
					),
	...
	$settings['wt_html_slider_min_height'] = array(
		...
		'section'     => $section,
		'type'        => 'text',
		'priority'    => 615, // Non static options must have a priority
		'default'     => 375,
		'description' => __('<strong>(in pixels)</strong><hr>Leave empty to let the slider height adjust automatically.', 'hoot-ubix'),
		'input_attrs' => array(
			'min' => 0,
			'max' => 3,
			'placeholder' => __( '(in pixels)', 'hoot-ubix' ),
	...
	for ( $slide = 1; $slide <= 4; $slide++ ) { 
		...
		$settings["wt_img_slide_{$slide}"] = array(
			...
			'section'     => $section,
			'type'        => 'group',
			'priority'    => 725, // Non static options must have a priority
			'button'      => sprintf( __( 'Edit Slide %s', 'hoot-ubix' ), $slide),
			'options'     => array(
				'description' => array(
					'label'       => '',
	...
	if ( current_theme_supports( 'woocommerce' ) ) :
		...
		$sections[ $section ] = array(
			'title'       => __( 'WooCommerce', 'hoot-ubix' ),
		...
		$settings['wooshop_products'] = array(
			'label'       => __( 'Total Products per page', 'hoot-ubix' ),
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '725', // Non static options must have a priority
			'choices'     => $wooproducts,
			'default'     => '12',
			'description' => __( 'Total number of products to show on the Shop page', 'hoot-ubix' ),
		);

		$settings['wooshop_product_columns'] = array(
			'label'       => __( 'Product Columns', 'hoot-ubix' ),
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '725', // Non static options must have a priority
			'choices'     => array(
			...
			'default'     => '3',
			'description' => __( 'Number of products to show in 1 row on the Shop page', 'hoot-ubix' ),
		);
...
function hootubix_theme_customizer_enqueue_scripts() {
	...
	// Enqueue Scripts
	wp_enqueue_script( 'hootubix-theme-customize-script', HYBRIDEXTEND_INCURI . 'admin/js/customize.js', array( 'jquery', 'wp-color-picker', 'customize-controls', 'hybridextend-customize-script' ), HYBRIDEXTEND_VERSION, true );
}
...
function hootubix_get_mod( $name, $default = NULL ) {
	...
	if ( is_customize_preview() ) {
		...
		if ( isset( $mods[$name] ) ) {
			...
			$override = apply_filters( 'hootubix_get_mod_override_empty_values', false, $name, $mods[$name] );
			if ( $override !== true )
				return apply_filters( 'hootubix_sanitize_get_mod', $mods[$name], $name );
		}
