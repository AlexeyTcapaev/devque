...
function hootubix_base_register_sidebars() {
	...
	if ( hootubix_get_mod( 'primary_menuarea' ) == 'widget-area' )
	hybrid_register_sidebar(
		array(
			'id'          => 'hootubix-header',
			'name'        => _x( 'Header Side', 'sidebar', 'hoot-ubix' ),
			'description' => __( 'Appears in Header on right of logo', 'hoot-ubix' )
		)
	);
	...
	if( $footercols ) :
		...
		for ( $i=0; $i < $footercols; $i++ ) :
			if ( isset( $alphas[ $i ] ) ) :
				hybrid_register_sidebar(
					array(
						'id'          => 'hootubix-footer-' . $alphas[ $i ],
						'name'        => sprintf( _x( 'Footer %s', 'sidebar', 'hoot-ubix' ), strtoupper( $alphas[ $i ] ) ),
						'description' => __( 'You can set footer columns in Theme Options page.', 'hoot-ubix' )
					)
				);
			endif;
		endfor;
...
function hootubix_frontpage_register_sidebars() {
	...
	foreach ( $areas as $key => $name ) {
		hybrid_register_sidebar(
			array(
				'id'          => 'hootubix-frontpage-' . $key,
				'name'        => $name,
				'description' => __( "You can order Frontpage areas in Customizer > 'Content' panel > 'Frontpage - Modules' section.", 'hoot-ubix' )
			)
		);
	}
