			...
			<script type="text/javascript">
				( function($){
					...
					<?php /*if (typeof window.hybridext_widget_helper["<?php echo get_class($this) ?>"] == 'undefined')*/ // This creates unexpected results as the script is first instancized in template widget __i__ ?>
						window.hybridext_widget_helper["<?php echo get_class($this) ?>"] = <?php echo json_encode( $this->repeater_html ) ?>;
					if (typeof $.fn.hybridextSetupWidget != 'undefined') {
						$('#<?php echo $form_id ?>').hybridextSetupWidget();
					}
				} )( jQuery );
			</script>
		</div><?php
	...
	function render_field( $field, $value, $repeater = array() ){
		...
		?><div class="hybridext-widget-field hybridext-widget-field-type-<?php echo ( strlen( $type ) < 15 ) ? sanitize_html_class( $type ) : 'custom' ?> hybridext-widget-field-<?php echo sanitize_html_class( $id ) ?>"><?php
			...
			switch( $type ) {
				...
				case 'icon':
					?><input id="<?php echo $this->hybridext_get_field_id( $id, $repeater ) ?>" class="hybridext-icon" name="<?php echo $this->hybridext_get_field_name( $id, $repeater ) ?>" type="hidden" value="<?php echo esc_attr( $value ) ?>" />
					<div id="<?php echo $this->hybridext_get_field_id( $id, $repeater ) . '-icon-picked' ?>" class="hybridext-icon-picked"><i class="fa <?php echo esc_attr( $value ) ?>"></i><span><?php _e( 'Select Icon', 'hybrid-core' ) ?></span></div>
					<div class="clear"></div>
					<div id="<?php echo $this->hybridext_get_field_id( $id, $repeater ) . '-icon-picker-box' ?>" class="hybridext-icon-picker-box">
						<div class="hybridext-icon-picker-list"><i class="fa fa-ban hybridext-icon-none" data-value="0" data-category=""><span><?php _e( 'Remove Icon', 'hybrid-core' ) ?></span></i></div>
						<?php
				...
				case 'color':
					...
					?><input id="<?php echo $this->hybridext_get_field_id( $id, $repeater ) ?>" class="hybridext-color" name="<?php echo $this->hybridext_get_field_name( $id, $repeater ) ?>" type="hidden" value="<?php echo esc_attr( $value ) ?>" <?php echo $default_color; ?> />
					<?php
					break;
					...
					?><div class="hybridext-widget-field-group" data-id="<?php echo esc_attr( $id ) ?>">
						<?php if ( !empty( $name ) ): ?>
							<div class="hybridext-widget-field-group-top">
								<h3><?php echo $name ?> <i class="fa fa-sort"></i></h3>
							</div>
						<?php endif; ?>
						<?php $item_name = isset( $options['item_name'] ) ? $options['item_name'] : ''; ?>
						<div class="hybridext-widget-field-group-items">
							<?php
							if ( !empty( $value ) ) {
						...
						?>
						<div id="add-<?php echo rand(1000, 9999); ?>" class="hybridext-widget-field-group-add" data-iterator="<?php echo is_array( $value ) ? max( array_keys( $value ) ) : 0; ?>"><?php _e('Add', 'hybrid-core') ?></div>
					</div>
					<?php
					break;
			...
			if ( ! empty( $desc ) )
				echo '<div class="hybridext-widget-field-description"><small>' . esc_html( $desc ) . '</small></div>';
