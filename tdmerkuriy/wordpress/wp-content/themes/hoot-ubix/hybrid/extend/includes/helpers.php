...
function hybridextend_trim_content( $raw, $words ) {
	...
	$content = apply_filters( 'hybridextend_trim_content', $content, $words, $excerpt_more );
	$content = apply_filters( 'wp_trim_excerpt', $content, $raw );
...
function hybridextend_get_image_size_name( $span, $crop=NULL, $gridadjust=0 ) {
	...
	$grid = ( function_exists( 'hootubix_get_mod' ) ) ? intval( hootubix_get_mod( 'site_width' ) ) : 0;
	if ( empty( $grid ) )
		$grid = $default_grid;
	$grid -= $gridadjust;
