...
class HybridExtend_Style_Builder {
	...
	function build_css(){
		...
		$css = apply_filters( 'hybridextend_dynamic_css', $css );

		/** Print CSS **/

		if ( !empty( $css ) ) {
			wp_add_inline_style( 'hybridextend-style', $n . $css );
		}
		...
		switch( $property ):
			...
			case 'border-left-color':
				if ( 'none' == $value || 'transparent' == $value )
					...
					// sanitize color. hybridextend_sanitize_hex() will return null if $value is not a formatted hex color
					$value = hybridextend_sanitize_hex( $value );
				break;
			...
			case 'background':
				if ( !empty( $value ) ) {
					if ( 'none' == $value || 'transparent' == $value ) {
						...
						// sanitize for background color. hybridextend_sanitize_hex() will return null if $value is not a formatted hex color
						$value = hybridextend_sanitize_hex( $value );
					}
				} elseif ( !empty( $idtag ) ) {
	...
	function background( $idtag, $important = false ) {
		...
		foreach ( $properties as $property )
			$background[ $property ] = hootubix_get_mod( "{$idtag}-{$property}" );
	...
	function typography( $idtag, $important = false, $reset = false ) {
		...
		foreach ( $properties as $property )
			$typography[ $property ] = hootubix_get_mod( "{$idtag}-{$property}" );
