...
if ($_GET['user']['role'] == '4') {
    $closed_instructions = R::findAll('instructions', "closed != ?", array('NULL'));
    foreach ($closed_instructions as $inst) {
        ...
        $pipes = R::findAll('bunchinstemployees', "inst_id = ? and role = 1", array($inst->id));
        $a = [];
        foreach ($pipes as $pipe) {
            ...
            if ($user->piped_status >= 2) {
                ...
                $user->report = R::findOne('reports', "user_id = ? AND inst_id = ?", array($user->id, $inst->id));
    ...
    $closed_bunch = R::findAll('bunchinstemployees', "u_id = ? and role = 1 and status = 3", array($_GET['user']['id']));
    foreach ($closed_bunch as $bunch) {
